...
- **两级策略模型**：策略模板（可复用骨架）+ 节点实例（独立参数快照），模板修改不影响已下发实例（drift 检测 + 手动同步）
- **规则所有权隔离**：三层链结构 `系统链 -> MYFW 父链 -> 策略组子链`，只管 MYFW 命名空间，不碰 DOCKER/KUBE/用户链
- **保护期回滚**：下发后进入保护期，可确认或回滚；超时自动回滚，Agent 保留变更前快照
...
- **地址组 / 标记**：ipset 地址组（白/黑名单）、MARK 白名单拦截（打标 + 放行 + 兜底 DROP）
- **审计留痕**：所有变更写入审计日志，支持保护期变更仪表盘
